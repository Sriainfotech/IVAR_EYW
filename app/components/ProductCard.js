...
export default function ProductCard({ product }) {
  ...
  return (
    <article className="group bg-white border border-[#e6e4dc] rounded-2xl relative transition-shadow duration-300 hover:shadow-[0_18px_40px_#17312618] flex flex-row items-stretch gap-3 p-3 md:flex-col md:items-stretch md:gap-0 md:p-0 md:overflow-hidden">
      ...
      <div className="order-2 md:order-1 relative shrink-0 w-24 h-24 md:w-full md:h-[260px]">
        <button
          onClick={() => openProduct(product)}
          className="block w-full h-full rounded-xl md:rounded-none bg-[#eef1e8] overflow-hidden"
        ...
        {comingSoon ? (
          <span
            className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 bg-[#f4f6f1] border-2 border-[#d6ddd7] rounded-md md:rounded-lg px-3 py-1 md:px-5 md:py-1.5 text-[8px] md:text-[10px] font-bold tracking-[0.06em] text-[#7d8983] shadow-[0_2px_8px_#17312622] whitespace-nowrap"
            suppressHydrationWarning
          >
            COMING SOON
          </span>
          ...
          <button
            className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 bg-white border-2 border-ivar-dark rounded-md md:rounded-lg px-3 py-1 md:px-6 md:py-1.5 text-[9px] md:text-[11px] font-bold tracking-[0.08em] text-ivar-dark shadow-[0_2px_8px_#17312622] transition-colors duration-200 hover:bg-ivar-dark hover:text-white whitespace-nowrap"
            onClick={(e) => {
              e.preventDefault();
              add(product.id);
            }}
